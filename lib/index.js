...
async function run() {
  try {
    ...
    const sourceIssue = context.sourceIssue || {node_id: testIssueId, user: {login: sourceRepo.owner.login}};
    ...
    // Loggin
    debug('source issue is %0', sourceIssue);
    debug('source repository is %0', sourceRepo);
    debug('label trigger is %0', sourceLabel);
    debug('router is set to %0', router);
    ...
    debug('retrieved target repo metadata %0', targetRepo.data);
    ...
    if (core.getInput('apply_label')) {
      ...
      debug('repo %s has labels %0', targetRepoName, labels.data);
