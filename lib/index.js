...
async function run() {
  try {
    ...
    if (core.getInput('test_issue')) {
      core.warn(`Test mode enabled. Forcing usage with issue ${core.getInput('test_issue')}`);
      debug('test mode enabled using issue %s', core.getInput('test_issue'));
    }

    // Get relevant things
    const sourceIssue = context.payload.issue || {node_id: core.getInput('test_issue')};
    debug('source issue is %o', sourceIssue);

    // @NOTE: we need to also provide a testing option for when ^ is actually labelled
