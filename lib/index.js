...
async function run() {
  try {
    ...
    if (testIssueId && testLabelName) {
      core.warn(`Test mode enabled. Forcing usage with issue:label ${core.getInput('test')}`);
      debug('test mode enabled using issue:label %s', core.getInput('test'));
    }

    // Get relevant things
    const router = core.getInput('router', {required: true});
    const sourceIssue = context.payload.issue || {node_id: testIssueId};
    ...
    if (sourceRepo === undefined) {
      throw new Error('Action must run on event that has a repository');
    }
